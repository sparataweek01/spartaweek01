...
def search():
    ...
    result2 = list(db.recipes.find({'ingredients': {"$regex": keyword}}))
    result3 = list(db.recipes.find({'process': {"$regex": keyword}}))
...
def recipe_detail():
    ...
    print(id_receive)
    recipe = db.recipes.find_one({'id': id_receive}, {'_id': False})
    print(recipe)
